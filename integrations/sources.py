...
# срабатывать на «ставки» (баг, найденный баттлтестом 20.09.2026).
COLD_WORDS = (
...
def _kw_match(low, kws):
    """Совпадение ключевых слов ниши. Короткие слова — по границе слова,
    иначе «ии» находится внутри «анализ»/«функции», а «1с» — внутри «1С-разработчик».

    Возвращает список СОВПАВШИХ ключей (без дублей по корню).
    """
    hits = []
    for k in (kws or []):
        k = k.strip().lower()
        if not k:
            continue
        if len(k) <= 4 or not k[-1].isalpha():
            # короткое или с цифрой: только как отдельное слово
...
def score_text(txt: str, kws=None):
    ...
    if kws:
        if len(niche) >= 2:
            ...
            return min(20, 5 + (10 if any(w in low for w in BUDGET_WORDS) else 0)), \
